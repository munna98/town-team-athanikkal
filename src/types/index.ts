...
export const createMemberSchema = z.object({
    ...
    aadhaarNo: z.string().regex(/^\d{12}$/, "Aadhaar must be exactly 12 digits").optional().or(z.literal("")),
    mobile: z.string().min(7, "Mobile number is too short").max(15, "Mobile number is too long").regex(/^[+]?[\d\s\-().]{7,15}$/, "Invalid mobile number"),
